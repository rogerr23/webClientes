...
export class AutenticarUsuarioComponent {
  ...
  autenticarUsuario() {
    ...
    this.httpClient.post('http://localhost:8084/api/usuarios/autenticar', this.formulario.value, { responseType: 'text' }).subscribe({
      next: (data) => {
        sessionStorage.setItem('token', data);

        sessionStorage.setItem('usuarios', this.formulario.value.email as string);
